...
const route = "/design-system/default/tokens/primary-tinted-background";
  ...
  return page.evaluate(() => {
    return Array.from(document.querySelectorAll("[data-token-variant-id]")).map((card) => {
      ...
      return {
        ...
        deniesStateMeaning: text.includes("selected") && text.includes("validation"),
      };
    });
  });
}
...
test.describe("primary tinted background token route", () => {
  test("desktop renders derived tint swatches with source dependency visible", async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.goto(route);

    await expect(page.getByRole("heading", { name: "Primary Tinted Background Tokens", level: 1 })).toBeVisible();
    await expect(page.locator(".token-spec-card")).toHaveCount(3);
    await expect(page.getByText("derived from the matching primary color source variant")).toBeVisible();
  ...
  test("temporary upstream HEX override updates derived tint preview without mutating signed values", async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.goto(route);

    const input = page.getByLabel("Preview primary HEX");
    const signedTintValue = page
      .locator("[data-token-variant-id='primary-tinted-background-original'] .token-spec-definition-grid")
      .getByText("color-mix(in srgb, #635bff 12%, white)")
      .first();
    const initialStyles = await diagnosticStyles(page);

    await expect(input).toHaveValue("#635bff");
    await expect(signedTintValue).toBeVisible();
  ...
  test("mobile and rtl keep derived tint proof readable and navigation operable", async ({ page }) => {
