...
describe("web app page settings integration flows", () => {
  it("TC-WEB-APP-PAGE-SETTINGS-INT-001 reads options updates settings and returns effective state for one curated page", async () => {
  ...
  it("TC-WEB-APP-PAGE-SETTINGS-INT-002 accepts governed design-system icon keys through the transport seam", async () => {
    const harness = createRootAuthIntegrationHarness();
    const hierarchyRepository = createInMemoryWebAppHierarchyRepository({
      modules: [createModuleRecord()],
