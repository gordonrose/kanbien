...
describe("web app page settings audit visibility", () => {
  it("TC-WEB-APP-PAGE-SETTINGS-AUD-001 keeps successful page-settings mutations operator-visible through deterministic responses", async () => {
  ...
  it("TC-WEB-APP-PAGE-SETTINGS-AUD-002 keeps denied page-settings mutations visible through platform security audit events", async () => {
    const harness = createRootAuthIntegrationHarness();
    const hierarchyRepository = createInMemoryWebAppHierarchyRepository({
      modules: [createModuleRecord()],
      pages: [createPageRecord()],
