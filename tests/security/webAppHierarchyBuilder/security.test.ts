...
describe("web app hierarchy builder security flows", () => {
  it("TC-WEB-APP-HIERARCHY-SEC-001 rejects missing or invalid authenticated sessions", async () => {
  ...
  it("TC-WEB-APP-HIERARCHY-SEC-002 enforces per-route capability gates", async () => {
    const harness = createRootAuthIntegrationHarness();
    mountWebAppHierarchyBuilderFeature(
      harness.app,
      harness,
  ...
  it("TC-WEB-APP-HIERARCHY-SEC-003 rejects unexpected system-managed fields on create", async () => {
    const harness = createRootAuthIntegrationHarness();
    mountWebAppHierarchyBuilderFeature(
      harness.app,
      harness,
