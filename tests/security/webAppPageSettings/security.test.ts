...
describe("web app page settings security flows", () => {
  it("TC-WEB-PAGE-SET-SEC-001 rejects missing sessions and enforces exact capability gates", async () => {
  ...
  it("TC-WEB-PAGE-SET-SEC-002 rejects unexpected fields and malformed options reads", async () => {
    const harness = createRootAuthIntegrationHarness();
    const hierarchyRepository = createInMemoryWebAppHierarchyRepository({
      modules: [createModuleRecord()],
      pages: [createPageRecord()],
