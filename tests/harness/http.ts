...
export async function invokeRaw<TBody = unknown>(
  app: Express,
  options: {
    method: "POST" | "PUT";
    path: string;
  ...
  request.connection = { remoteAddress: "127.0.0.1", destroy() {} };
  request.socket = { remoteAddress: "127.0.0.1", destroy() {} };
  request.ip = "127.0.0.1";
  request.protocol = "http";
  request.get = (name: string) => request.headers[name.toLowerCase()];
  request.header = request.get;
  ...
  return new Promise<Omit<JsonResponse, "body"> & { body: TBody }>((resolve, reject) => {
    response.on("finish", () => {
      const rawBody = Buffer.concat(chunks).toString("utf8");
      const contentType = response.getHeader("content-type");
      const shouldParseJson =
