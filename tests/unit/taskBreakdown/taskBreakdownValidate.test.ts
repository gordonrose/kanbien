...
const backendGuardrailEvidenceRows =
  ...
  "| T-S001-01 | backend-source-inventory | pass | Backend source inventory names feature contract, domain, transport, persistence, and focused test files. |\n" +
  ...
  "| T-S001-01 | backend-expected-output | pass | Expected backend behavior output is named. |\n" +
  ...
  "| T-S001-01 | backend-human-review-boundary | pass | Human review is limited to backend behavior sufficiency and split routing. |";
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-S001-01 | domain-behavior | Story S-001, capability CAP-BRANDING-001, API contract, permission mapping, and data dictionary authorize this backend behavior. | src/features/tenantConfiguration | new-capability-file | src/features/tenantConfiguration/contract/; src/features/tenantConfiguration/domain/updateBranding.ts; src/features/tenantConfiguration/transport/rootAdminRoutes.ts; tests/integration/tenantConfiguration/persistence.test.ts | exact files: src/features/tenantConfiguration/domain/updateBranding.ts; src/features/tenantConfiguration/transport/rootAdminRoutes.ts; tests/integration/tenantConfiguration/persistence.test.ts | domain/updateBranding.ts; transport/rootAdminRoutes.ts; tests/integration/tenantConfiguration/persistence.test.ts | domain owns branding rule, transport routes, persistence test proves repository behavior | approved API contract; no wire change invented in DEV:backend | root authz required; tenant context from route; allow and deny proof required; lifecycle not-applicable | existing repository and persistence behavior consumed; no schema, migration, or index change | no new public seam; manifest unchanged unless export changes | API contract, permission mapping, data dictionary, feature docs, generated artifacts reviewed or split | not-applicable: manual logic only for task-specific domain behavior | expected behavior: selected tenant branding display name persists through approved backend route | not-applicable: API, permission, data dictionary, migration, platform, test-only, and evidence work unchanged or already split | npx vitest run tests/integration/tenantConfiguration/persistence.test.ts | repo formatter and no generated graph change expected | human review limited to backend behavior sufficiency and split routing |
...
describe("task breakdown validation", () => {
  ...
  it("allows API contract tasks with explicit route shape, artifact posture, and split routing", () => {
    const apiContractRow =
      "| T-S001-01 | additive-route-contract | root-admin tenant configuration | Source story AC-S001-01 and existing API contract docs authorize the route contract update. | PATCH /v1/root-admin/tenants/:tenantId/branding | route param tenantId required; request body displayName required | response includes updated tenant branding; status 200; validation errors return 400 | root auth required; CAP-BRANDING-001 authz; tenant boundary from route param tenant context | validation rejects empty displayName; pagination not-applicable; sorting not-applicable; system-managed fields not client supplied | additive | openapi-and-postman-maintained | docs/api-contracts/tenant-configuration.md; openapi/root-admin.yaml; postman/root-admin.json | not-applicable: docs-only contract update; runtime, permission mapping, migrations, and tests already split or unchanged | human review limited to contract wording and compatibility judgment | contract review plus OpenAPI/Postman validation |";
  ...
  it("blocks API contract tasks with compatibility-sensitive changes and no compatibility route", () => {
    const apiContractRow =
      "| T-S001-01 | compatibility-sensitive-contract | root-admin tenant configuration | Source story AC-S001-01 and existing API contract docs authorize the route contract update. | PATCH /v1/root-admin/tenants/:tenantId/branding | route param tenantId required; request body displayName required | response includes updated tenant branding; status 200; validation errors return 400 | root auth required; CAP-BRANDING-001 authz; tenant boundary from route param tenant context | validation rejects empty displayName; pagination not-applicable; sorting not-applicable; system-managed fields not client supplied | compatibility-sensitive | openapi-maintained | docs/api-contracts/tenant-configuration.md; openapi/root-admin.yaml | not-applicable: docs-only update | human review limited to compatibility judgment | contract review |";
  ...
  it("blocks API contract tasks that route runtime work without a DEV split", () => {
    const apiContractRow =
      "| T-S001-01 | additive-route-contract | root-admin tenant configuration | Source story AC-S001-01 and existing API contract docs authorize the route contract update. | PATCH /v1/root-admin/tenants/:tenantId/branding | route param tenantId required; request body displayName required | response includes updated tenant branding; status 200; validation errors return 400 | root auth required; CAP-BRANDING-001 authz; tenant boundary from route param tenant context | validation rejects empty displayName; pagination not-applicable; sorting not-applicable; system-managed fields not client supplied | additive | docs-api-contract-only | docs/api-contracts/tenant-configuration.md | runtime route handler follow-up needed | human review limited to contract wording | contract review |";
  ...
  it("blocks data dictionary retention review debt without a disposition owner", () => {
    const dataDictionaryRow =
      "| T-S001-01 | tenantConfiguration tenant_branding displayName durable fact | docs/data-dictionary/tenant-configuration.md | Source story AC-S001-01 and migration/schema reviewed. | field displayName and index posture recorded | durable fact retained; retention posture recorded | data classification internal; audit and retention posture recorded | repo durable data standard applies | manual-review | docs/data-dictionary/tenant-configuration.md evidence; npm run data:compliance-health | existing persistence test evidence recorded | docs-only-alignment | not-applicable: docs-only data dictionary update; schema, repository/domain runtime, API, permission mapping, standards, and tests unchanged | manual dictionary review plus npm run data:compliance-health |";
    ...
    const result = validateTaskBreakdownContent(
      validTaskPacket
        ...
        .replace(
          "## Debt Health Summary Commands\n\n| Task ID | Summary Command | Summary Result | Debt Found | Debt Disposition | Follow-Up Task ID / Owner |\n| --- | --- | --- | --- | --- | --- |\n\n",
          "## Debt Health Summary Commands\n\n| Task ID | Summary Command | Summary Result | Debt Found | Debt Disposition | Follow-Up Task ID / Owner |\n| --- | --- | --- | --- | --- | --- |\n| T-S001-01 | npm run data:compliance-health | debt-found | 53 manual-review-required retention/export/delete/legal-hold rows remain | none | not-applicable: no follow-up |\n\n",
        ),
      sourceStoryPacket,
    );

    expect(result.status).toBe("BLOCKED");
    expect(result.errors).toContain("T-S001-01 DOC:data-dictionary retention/export/delete/legal-hold review debt must be resolved in scope, split, accepted with owner, or blocked");
    expect(result.errors).toContain("T-S001-01 DOC:data-dictionary retention/export/delete/legal-hold review debt must name a data, standards, or governance owner");
  });
  ...
  it("blocks DEV:platform-seam tasks without a platform seam class contract row", () => {
    ...
    expect(result.status).toBe("BLOCKED");
    expect(result.errors).toContain("T-S001-01 has no Platform Seam Class Contract row");
  });
  ...
  it("blocks platform seam class rows that do not match the seam kind", () => {
    ...
    const result = validateTaskBreakdownContent(platformSeamPacketWithContract(row, classRow), sourceStoryPacket);

    expect(result.status).toBe("BLOCKED");
    expect(result.errors).toContain("T-S001-01 Platform Seam Class Contract class must match the Platform Seam Contract seam kind");
  });
  ...
  it("blocks router platform seam class rows without route consumer proof", () => {
    ...
    expect(result.status).toBe("BLOCKED");
    expect(result.errors).toContain("T-S001-01 router-route-mounting platform seam class must prove route registration/mounting and an existing route consumer");
  });
  ...
  it("blocks extraction when the dependent task does not depend on the extraction task", () => {
    const packetWithExtractionTask = validTaskPacket
      ...
      .replace(
        "## Refactor-First Contract\n\n| Task ID | Refactor Trigger | Refactor Type | Refactor Target Inventory | Detection Hints | Unchanged Behavior | Affected Consumers | Downstream Task Unblocked | Compatibility Proof | Routing Check | Human Review Boundary | Forbidden Behavior / Authority Change |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
        "## Refactor-First Contract\n\n| Task ID | Refactor Trigger | Refactor Type | Refactor Target Inventory | Detection Hints | Unchanged Behavior | Affected Consumers | Downstream Task Unblocked | Compatibility Proof | Routing Check | Human Review Boundary | Forbidden Behavior / Authority Change |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n| T-S001-00 | extraction-before-reuse | extract | src/features/tenantConfiguration/domain/service.ts; tests/unit/tenantConfiguration/brandingNormalization.test.ts | rg \"normalizeBranding displayName\" src/features/tenantConfiguration tests/unit/tenantConfiguration | Existing branding normalization returns the same normalized display-name value. | Existing tenantConfiguration callers and tests. | T-S001-01 | Existing consumer compatibility protected by branding normalization regression. | stays-refactor-first | human review limited to behavior-preserving refactor pressure and target inventory | Product behavior, API contract, persistence meaning, architecture authority, and shared seam authority must not change. |",
      )
      .replace(
        "| T-S001-01 | codex/s001-tenant-branding-backend | current dedicated task branch | docs/workspace/chat-bootstraps/2026-04-29-s001-tenant-branding-backend.md | origin/main | record exact base commit before Delivery edits | main after promote guardrail |",
  ...
  it("blocks refactor-first tasks when the routing check routes to an owning task type", () => {
    const refactorPacket = validTaskPacket
      ...
      .replace(
        "## Refactor-First Contract\n\n| Task ID | Refactor Trigger | Refactor Type | Refactor Target Inventory | Detection Hints | Unchanged Behavior | Affected Consumers | Downstream Task Unblocked | Compatibility Proof | Routing Check | Human Review Boundary | Forbidden Behavior / Authority Change |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
        "## Refactor-First Contract\n\n| Task ID | Refactor Trigger | Refactor Type | Refactor Target Inventory | Detection Hints | Unchanged Behavior | Affected Consumers | Downstream Task Unblocked | Compatibility Proof | Routing Check | Human Review Boundary | Forbidden Behavior / Authority Change |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n| T-S001-01 | extraction-before-reuse | extract | src/features/tenantConfiguration/domain/service.ts; tests/unit/tenantConfiguration/brandingNormalization.test.ts | rg \"normalizeBranding displayName\" src/features/tenantConfiguration tests/unit/tenantConfiguration | Existing normalization behavior remains unchanged. | Existing tenantConfiguration consumers. | not-applicable: downstream task not in this negative fixture | Existing consumer compatibility protected by regression proof. | blocked-route-to-DEV:platform-seam | human review limited to routing away platform seam authority | Public platform seam authority must not change inside refactor-first. |",
      );
  ...
  it("accepts a specific architecture-foundation blocker with an authority route", () => {
    const architecturePacket = validTaskPacket
      ...
      .replace(
        "## Architecture Foundation Contract\n\n| Task ID | Concern Area | Architecture Trigger | Architecture Question | Decision Analysis Status | Decision Provenance Source | Missing Analysis Fields | Sources To Review | Decision Source Inventory | Decision Analysis Checklist | Decision Owner | Output Artifact Target | Downstream Tasks Blocked | Compatibility Posture | Final Authority Route | Human Review Boundary | Forbidden Implementation / Guess |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
        "## Architecture Foundation Contract\n\n| Task ID | Concern Area | Architecture Trigger | Architecture Question | Decision Analysis Status | Decision Provenance Source | Missing Analysis Fields | Sources To Review | Decision Source Inventory | Decision Analysis Checklist | Decision Owner | Output Artifact Target | Downstream Tasks Blocked | Compatibility Posture | Final Authority Route | Human Review Boundary | Forbidden Implementation / Guess |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n| T-S001-00 | ownership-boundary | owner-boundary | Does tenant branding ownership remain in tenantConfiguration for this backend change? | approved-source-exists | Technical Steering DEC-001 approved analysis. | none | docs/architecture/adr/; docs/architecture/system-overview.md; Technical Steering packet | docs/architecture/adr/; docs/architecture/system-overview.md; docs/workspace/technical-steering/tenant-branding-ownership.md | options, trade-offs, risk, cost, compatibility, operability, testability, reversibility, recommendation, signoff reviewed in Technical Steering DEC-001 | Technical Steering | docs/workspace/technical-steering/tenant-branding-ownership.md or docs/architecture/adr/new-tenant-branding-ownership.md | T-S001-01 | Backwards compatibility required; no migration or runtime behavior change in this task. | GOV:architecture-update | human review limited to architecture sufficiency and final authority routing | Do not implement backend behavior, change API contracts, or invent architecture ownership. |",
      )
      .replace(
        "| T-S001-01 | codex/s001-tenant-branding-backend | current dedicated task branch | docs/workspace/chat-bootstraps/2026-04-29-s001-tenant-branding-backend.md | origin/main | record exact base commit before Delivery edits | main after promote guardrail |",
