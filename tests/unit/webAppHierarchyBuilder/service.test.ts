...
describe("web app hierarchy builder service", () => {
  it("TC-WEB-APP-HIERARCHY-UNIT-001 creates a module-root page and derives its full route path", async () => {
  ...
  it("TC-WEB-APP-HIERARCHY-UNIT-002 blocks route-affecting moves for live branches", async () => {
    const repository = createInMemoryWebAppHierarchyRepository({
      modules: [createModuleRecord()],
      pages: [
        createPageRecord({
  ...
  it("TC-WEB-APP-HIERARCHY-UNIT-003 rejects duplicate page keys during bootstrap and normal create flows", async () => {
    const repository = createInMemoryWebAppHierarchyRepository({
      modules: [createModuleRecord()],
