...
| --- | --- | --- | --- | --- |
| Durable domain data rule | yes | enforced-by-maintained-artifact | This data dictionary page; `AGENTS.md` durable domain data rule | Tenant Auth Principal is documented as owned by `tenantAuth` with source record(s) `tenant_auth_principal`, `TenantAuthPrincipalRecord`. |
