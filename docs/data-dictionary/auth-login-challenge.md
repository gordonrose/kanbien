...
| --- | --- | --- | --- | --- |
| Durable domain data rule | yes | enforced-by-maintained-artifact | This data dictionary page; `AGENTS.md` durable domain data rule | Auth Login Challenge is documented as owned by `rootAuth` with source record(s) `auth_login_challenges`, `AuthLoginChallengeRecord`. |
