...
| --- | --- | --- | --- | --- |
| Durable domain data rule | yes | enforced-by-maintained-artifact | This data dictionary page; `AGENTS.md` durable domain data rule | Tenant Branding Logo Relationship is documented as owned by `tenantBranding` with source record(s) planned `tenant_branding_logo_relationship`, planned `TenantBrandingLogoRelationshipRecord`. |
