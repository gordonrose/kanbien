...
| Owning feature | planned `organizationExports` |
| Ownership status | `planned` |
| Current entity status | `draft` |
| Primary authority | `planning-artifact` |
| Primary source table or record | planned `organization_export`, planned `OrganizationExportRecord` |
| Entity definition lineage | `not-yet-registered` |
| Latest source review date | `2026-05-14` |
| Related PRD / steering / ADR | `docs/prd/2026-05-12-0025-organization-domain-foundation.md`; `docs/workspace/asset-consumer-decisions/2026-05-12-organization-private-export-bundle.md`; `docs/api-contracts/organization-root-admin.md`; `docs/api-contracts/organization-tenant-admin.md` |
...
| Durable fact boundary | Actor and tenant/account authority, selected sections, job status, expiry, size/checksum, download limits, failure category, cleanup state, and source-data/export-copy separation. |
| Primary users / actors | Root admins, tenant admins, background export worker, cleanup worker, support/audit reviewers. |
| Rebuild-from-spec value | A future maintainer can reconstruct export records, limits, background job posture, private download behavior, expiry/delete cleanup, and evidence requirements. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `organization.export.create` | `import-export` | create export request | root, tenant | UI/API/job | creates background export request | audit create; rate/limit proof | private export decision; API contracts | Background job only. |
...
| `organization.export.download` | `import-export` | download ready private ZIP | root, tenant | UI/API/export | increments download attempt count | download audit; limit proof | private export decision | Private attachment, no raw storage URL. |
...
| `status` | `status` | `lifecycle` | `queued`, `running`, `ready`, `failed`, `expired`, or `deleted` | `single` | yes | yes | lifecycle-only | exact, facet | status badge | private export decision |
...
| `download_attempt_count` | `download_attempt_count` | `security` | `INTEGER` | `single` | yes | yes | system-only | range, sort | download limit counter | private export decision |
...
| `security` | Actor authority, download count, storage reference. | Restricted/read-only. | Never raw storage URL; enforce limits. | The value affects access or abuse controls. |
...
| `active` | Export request is queued, running, or ready before expiry/delete. | authorized export reads | status read, download if ready, delete | private export decision |
...
| `export.actor` | `reference` | Organization Export | root or tenant actor | many-to-one | Actor limits and audit attribution required. | Actor request/download limits apply. | requester metadata | private export decision |
...
| `ix_organization_export_actor_active` | `index` | `actor_id`, `status` | Actor active export lookup. | Enforces 1 active export per actor. | private export decision |
| `organization_export_active_limits` | `code-enforced validation` | actor, tenant, platform active counts | 1 active per actor, 3 per tenant, 10 platform-wide. | Prevents abuse/cost spikes. | API contracts |
| `organization_export_rate_limits` | `code-enforced validation` | actor, tenant request counts | 5 requests per actor per hour, 20 per tenant per day. | Prevents abuse/cost spikes. | API contracts |
| `organization_export_size_limit` | `code-enforced validation` | generated ZIP size | ZIP size cap is 250 MB. | Controls storage/transfer risk. | API contracts |
| `organization_export_download_limit` | `code-enforced validation` | download attempt count | 10 download attempts before expiry. | Controls abuse/transfer. | API contracts |
...
| `export.active-limits` | actor, tenant, platform counts | 1 active per actor, 3 per tenant, 10 platform-wide. | `ORGANIZATION_EXPORT_LIMIT_EXCEEDED`. | API contracts |
| `export.rate-limits` | actor and tenant request counts | 5 per actor per hour and 20 per tenant per day. | rate-limit denial. | private export decision |
...
| `actor_id` | exact, facet | scalar | actor active index | Supports actor limits and audit. | private export decision |
...
| --- | --- | --- | --- | --- | --- |
| `create` | root/tenant via export create | selected sections | stamps ID, actor, tenant, queued status, timestamps; enqueues job | Enforce active/rate limits. | API contracts |
| `generate` | system background job | status, size, checksum, storage reference, failure category | marks running/ready/failed; records attempts | Must verify source data and generated bytes. | private export decision |
| `download` | root/tenant via export download | download attempt count | records download attempt | Must be ready, unexpired, and under attempt cap. | API contracts |
...
| Export | ZIP contains selected CSV/JSON structured sections and actual uploaded logo image files where selected. | `organizationExports` | export job | failures record safe failure category. | private export decision | private export decision |
...
| Object-level rule | Export request/status/download/delete denied unless actor is authorized for target tenant/account and export record. | private export decision |

## API, UI, And Design-System Posture

| Concern | Posture | Source |
| --- | --- | --- |
| API required | planned export request/status/download/delete APIs | API contracts |
...
| List view treatment | export history/status list with expiry and availability | API contracts |
| Detail view treatment | selected sections, status, size/checksum, failure category, download availability | API contracts |
| Create/edit treatment | create-only section selector and confirmation | API contracts |
| Lifecycle action treatment | delete/unavailable confirmation and expiry indicators | private export decision |
...
| Security relevance | high because private downloads, raw URL denial, limits, checksums, and cleanup are required | private export decision |
| Audit relevance | yes for create, job start/complete/fail, download, delete, cleanup failure, and denials | private export decision |
...
| Operational evidence requirements | `npm run data:compliance-health`; future job/limit/download/checksum/cleanup tests and runbook | this page |
...
| `ORGANIZATION_EXPORT_LIMIT_EXCEEDED` | export limit exceeded. | request | active/rate/size limit exceeded | API contracts |
...
| `ORGANIZATION_EXPORT_DOWNLOAD_LIMIT_EXCEEDED` | export download limit exceeded. | `exportId` | download attempt count exceeded | API contracts |
...
| asset decision | `docs/workspace/asset-consumer-decisions/2026-05-12-organization-private-export-bundle.md` | Export generation, limits, retention, cleanup, security, and legal-hold posture. |
