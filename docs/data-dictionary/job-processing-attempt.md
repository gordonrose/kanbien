...
| --- | --- | --- | --- | --- |
| Durable domain data rule | yes | enforced-by-maintained-artifact | This data dictionary page; `AGENTS.md` durable domain data rule | Job Processing Attempt is documented as owned by `jobProcessing` with source record(s) `job_processing_attempt`, `JobProcessingAttemptRecord`. |
