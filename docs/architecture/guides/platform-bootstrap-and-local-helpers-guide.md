...
- asset local storage configuration through optional `ASSETS_LOCAL_STORAGE_ROOT`
...
5. set `REDIS_URL` when using a non-default local Redis endpoint
6. optionally set `ASSETS_LOCAL_STORAGE_ROOT` to choose where local/dev/test
   asset objects are stored; when omitted, the app uses `.local-assets` under
...
The asset foundation v1 uses a local filesystem object-storage adapter for
local/dev/test until a production S3-compatible provider is selected.

Current local setting:

- `ASSETS_LOCAL_STORAGE_ROOT`
  optional directory for object bytes and sidecar metadata

Operational expectations:

- do not commit local object bytes or generated storage metadata
- storage keys are generated by the server and must not use raw filenames as
  filesystem authority
- same-origin content reads go through `/v1/assets/{assetId}/content`
- production provider lifecycle rules for aborting incomplete multipart uploads
  remain a follow-up once the provider is selected
