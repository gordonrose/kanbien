...
| Element compatibility | Element choices must be validated against the surface, region, sub-region/component contract, attribute type, cardinality, mutability, and other governed metadata. |
...
| Group order | Groups should carry their own display order. Attribute placement order is scoped within the relevant surface, region, sub-region, and group; for ungrouped areas it is scoped to the surface, region, and sub-region. |
...
| Placement key configurability | `surfaceKey`, `surfaceVariantKey`, `regionKey`, `subRegionKey`, and `elementKey` may be configurable only through approved design-system contracts. Entity definitions select from approved keys and combinations; they must not invent entity-local slot, region, sub-region, variant, or element names. |
...
| Collection views | Add a governed collection-view model under `surfaceModel`. A collection view defines an approved way to see a collection, which role groups may use it, which operational statuses and sub-statuses belong to it, whether it is default, and which template regions such as `view_selector` and `status_bar` may render it. Views are entity-definition metadata, not ad hoc page tabs. |
...
| Design-system boundary | Whether a surface/region/sub-region/element supports an interaction mode such as hidden-by-default should be validated through the design-system component contract, not hard-coded as entity-specific UI logic. |
...
| The existing form-pattern catalog already governs durable attribute control references and ties them to signed-off design-system seams. | `docs/workspace/entity-definitions/approved-form-pattern-catalog.md`; `src/features/entityBuilder/domain/catalogs.ts` | Our `elementKey`/surface placement model should reuse this posture and extend it from form-only controls to surface/region/sub-region/element contracts. |
...
| Current attribute ordering is one global `displayOrder` per attribute/version, with a unique constraint. Our planned model uses scoped placement order by surface/region/sub-region/group. | `0014_create_entity_builder_foundation.sql`; presentation decisions in this tracker | This is a real migration issue. The richer model should preserve/translate current global order into default placements rather than breaking historical exports. |
...
| Global `displayOrder` migration | Existing rows require one display order per attribute; planned placements need scoped order. | Decision locked: map current `displayOrder` into default surface placement order during migration. New canonical ordering lives on placements scoped by surface, region, sub-region, and group. |
...
| `subRegionKey` | Approved nested region for the surface/region, or `none`. |
| `elementKey` | Approved design-system element/control/display key. |
| `groupKey` | Group key or `none`. |
| `displayOrder` | Order within the surface/region/sub-region/group. |
...
| `collectionViews` | Approved collection views, including role eligibility and status/sub-status membership. |
...
Each collection view should include a stable view key, label/description
localization keys with fallback copy, default-view posture, role eligibility,
included status keys, included sub-status keys when applicable, status display
posture, template-region eligibility, and evidence keys. A status or sub-status
can belong to more than one view only when the view definitions make the overlap
explicit.
