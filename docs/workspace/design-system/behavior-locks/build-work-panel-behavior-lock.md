...
| `BWP-015` | Conversation history must support a fast new-chat action, editable chat titles, hover/focus row actions for archive and rename, reversible archive with undo, and a recoverable archived view. | Keeps the history lane useful for repeated discovery work without turning the list into a bulky management screen or making archive feel destructive. | Canonical states `BWP-R-021` and `BWP-R-022` prove the shared renderer/controller seam for active history actions and archived-history recovery. | `approved` | User approved instant new chat, editable titles, immediate archive with undo, and an archived view inside the history panel. |
...
- The API/data model for editable conversation titles, archive/restore
  lifecycle, packet-chain history, and root-builder authorization.
