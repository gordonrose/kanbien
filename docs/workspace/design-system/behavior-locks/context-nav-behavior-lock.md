...
| `SV-001B` | The bottom utility zone may host persistent shell utilities that are not page destinations, including an async activity launcher for background job progress, but those utilities must remain visibly separate from primary navigation. | Lets backend-triggered asynchronous work stay visible while users keep navigating, without turning job progress into a route-local page concern. | The page-shell template now places `#async-activity-button` in `.context-nav-bottom-group` alongside the display drawer launcher and opens a shell-attached async activity drawer. | `approved` | Async job status belongs in the persistent bottom utility stack, next to display settings. |
