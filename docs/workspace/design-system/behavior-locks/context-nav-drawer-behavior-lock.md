...
| `CD-012` | An async activity drawer launched from the context-nav bottom stack may show one or many background jobs and must preserve shell-level visibility while the user continues working elsewhere. | Gives asynchronous backend work a durable frontend status surface without blocking navigation or scattering per-page progress widgets. | The page-shell template now includes `#async-activity-drawer`, with a live summary, multiple job cards, and progress indicators using the same shell-attached drawer chassis as display settings. | `approved` | The async drawer is a persistent job-status tool, not a modal task blocker. |
