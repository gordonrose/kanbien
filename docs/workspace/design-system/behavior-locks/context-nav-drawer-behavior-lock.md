...
| `CD-012` | An async activity drawer launched from the context-nav bottom stack may show one or many background jobs, including in-progress, waiting, error/retry, and completed job result states with a CSV download action for failure/success reporting, while preserving shell-level visibility as the user continues working elsewhere. | Gives asynchronous backend work a durable frontend status surface without blocking navigation or scattering per-page progress widgets. | The page-shell template now mounts the shared `async-activity-drawer` seam from `src/frontend/designSystem/assets/asyncActivityDrawer.mjs`, with multiple job cards, progress indicators, an error state with retry action, a complete state with successful and failed record counts, and a compact CSV download action using the same shell-attached drawer chassis as display settings. | `approved` | The async drawer is a persistent job-status tool, not a modal task blocker. |
