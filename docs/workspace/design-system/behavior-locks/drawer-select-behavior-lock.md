...
| --- | --- | --- | --- | --- | --- |
| `DS-001` | `Drawer Select` must remain a child seam hosted inside the signed-off `Form Template` parent and must inherit the parent page framing instead of redefining page layout, section rhythm, or action zoning. | Keeps the seam reusable without letting extraction absorb parent ownership. | The current route hosts both drawer-select variants inside the existing form sections and action rails without changing the parent shell. | `approved` | Treat this as a child seam, not a parent page. |
