...
| --- | --- | --- | --- | --- | --- |
| `CG-BL-001` | `Choice Group` must be governed as a child seam of `Form Template` rather than as a replacement for the whole parent page, section, or field tile. | Keeps extraction honest and prevents the child seam from absorbing page rhythm too early. | The current grouped-choice surfaces use the same tile host language as field-level seams while living inside the stable `Preferences` section; the page shell and action rails remain parent-owned. | `approved` | Treat this as a child seam, not a parent page. |
