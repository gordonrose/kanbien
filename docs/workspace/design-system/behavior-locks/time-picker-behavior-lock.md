...
| --- | --- | --- | --- | --- | --- |
| `TP-BL-001` | The `Time Picker` must be governed as a child seam of `Form Template` rather than as a replacement for the whole parent page or field tile. | Keeps extraction honest and prevents the child seam from silently absorbing parent page ownership. | The current time pickers live inside stable `.form-field` tile hosts while the page shell, section cadence, and action rails stay parent-owned. | `approved` | Treat this as a child seam, not a parent page. |
