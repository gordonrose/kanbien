...
| `HTR-022` | `/design-system/canonical-renderings/hierarchy-tree/HTR-022` | Mobile row menu structural-edit state | Preserves the menu-only mobile action model once the drawer fills the screen | covered-by-test | Generated-route proof verifies hidden inline hover actions and visible mobile row-menu open actions |
...
| `HTR-024` | `/design-system/canonical-renderings/hierarchy-tree/HTR-024` | RTL mirrored drawer docking and mirrored expander placement | Preserves native-feeling directionality at both shell and row levels | covered-by-test | Generated-route proof verifies expander/content/action geometry and right-aligned row copy |
...
| `HTR-026` | `/design-system/canonical-renderings/hierarchy-tree/HTR-026` | Dark-theme readability and row/control contrast review | Preserves legibility of text, boundaries, states, and controls under non-default theme pressure | covered-by-test | Generated-route proof verifies dark theme scope plus readable selected-row title and menu control color against a dark row surface |
...
| `HTR-030` | `/design-system/canonical-renderings/hierarchy-tree/HTR-030` | Desktop long-title overflow review | Preserves clean truncation and control access when page titles exceed the resting row width | covered-by-test | Generated-route proof verifies truncation geometry, tooltip preservation, and row-menu access beside long titles |
...
- focused executable proof now includes the generated index-to-family-to-render
  launcher chain; generated `HTR-022` mobile row-menu, `HTR-024` RTL docking,
  `HTR-026` dark readability, and `HTR-030` long-title overflow route truth;
  and a generated launcher href check for that priority batch
