...
| --- | --- | --- | --- | --- | --- |
| `LDSLR-001` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-001` | Desktop closed baseline | Preserves the one-lane closed state before detail opens | canonical-created | Split lane absent when nothing is selected |
| `LDSLR-002` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-002` | Desktop open split baseline | Preserves the standard pushed two-lane relationship | canonical-created | First child review anchor |
| `LDSLR-003` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-003` | Independent scroll-lane pressure | Preserves the rule that list and detail remain separate reading lanes under longer content | canonical-created | Dedicated split-lane pressure state |
| `LDSLR-004` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-004` | Mobile full-sheet overlay | Preserves the narrow-width overlay reading model | canonical-created | Child mobile layout proof |
| `LDSLR-005` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-005` | Mobile overlay beneath shell chrome | Preserves the layering rule that shell overlays stay above the mobile detail sheet | canonical-created | Child layering proof |
| `LDSLR-006` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-006` | RTL desktop split review | Preserves mirrored lane placement in RTL | canonical-created | Child-level RTL proof |
| `LDSLR-007` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-007` | Magnified half-page split review | Preserves the split relationship under tighter width and zoom pressure | canonical-created | Child magnification proof |
| `LDSLR-008` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-008` | Dark theme open split | Preserves lane readability under dark theme | canonical-created | Theme scope stays local |
| `LDSLR-009` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-009` | Desert theme open split | Preserves parity beyond the default palette | canonical-created | Theme scope stays local |
| `LDSLR-010` | `/design-system/canonical-renderings/list-detail-split-layout/LDSL-010` | Squashed-split fallback review | Must prove that when both lanes become too constrained, the seam falls back to the approved overlay or single-lane posture instead of preserving an unreadable split | canonical-created | Non-mobile fallback proof under stronger width and zoom pressure |
