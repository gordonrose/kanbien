...
| `build-work-panel` | `patterns` | Build-specific configuration of the conversation panel with Reporting/Support/Build toolbar, Build chat copy, and Product Discovery packet PDF journey | `system-ready` | Signed-off pattern direction on `/design-system/patterns/build-work-panel-demo`; consumes shared `conversationPanel` render/controller/style seams; canonical routes `BWP-R-001` through `BWP-R-020` under `/design-system/canonical-renderings/build-work-panel/*`; Playwright canonical coverage in `tests/visual/designSystem/canonicals/shell/buildWorkPanel.spec.ts`; root-admin adoption coverage in `tests/visual/app/rootAdminShell/rootAdminShellParity.spec.ts`; live runtime evidence records protected harness chat, OpenAI-backed assistant response, and persisted usage attempts | Keep Reporting and Support inactive until their product flows are approved; prove a second governed consumer before broadening the primitive | Root-admin shell Build panel |
