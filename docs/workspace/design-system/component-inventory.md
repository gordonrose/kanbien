...
| `build-work-panel` | `patterns` | Shell-attached Reporting/Support/Build work panel with Build chat, right-side icon toolbar, collapsible conversation history, composer tools, message actions, display settings review controls, and packet PDF journey | `system-ready` | Signed-off pattern direction on `/design-system/patterns/build-work-panel-demo`; shared render/controller seam in `/design-system/assets/buildWorkPanel.mjs`; canonical routes `BWP-R-001` through `BWP-R-020` under `/design-system/canonical-renderings/build-work-panel/*`; app adoption remains blocked pending first-consumer parity | Add full Playwright canonical scenario batch, harden app style-entrypoint naming, and complete root-admin first-consumer parity review before adoption | Root-admin shell Build panel |
