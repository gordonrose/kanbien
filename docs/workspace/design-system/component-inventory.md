...
| `conversation-panel` | `patterns` | Reusable shell-attached conversation surface with history, transcript, composer tools, message actions, responsive panel framing, and packet/action extension points | `system-ready` | Shared render/controller/config seam in `/design-system/assets/conversationPanel.mjs`; shared style seam in `/design-system/assets/conversationPanel.css`; first configured canonical family is `build-work-panel`; root-admin consumes the seam for UI-only adoption with temporary local handlers | Connect real Layer 1/harness behavior for the first consumer and add a second non-Build configuration before broader promotion | Build harness, Support, team chat, future conversation surfaces |
| `build-work-panel` | `patterns` | Build-specific configuration of the conversation panel with Reporting/Support/Build toolbar, Build chat copy, and Product Discovery packet PDF journey | `system-ready` | Signed-off pattern direction on `/design-system/patterns/build-work-panel-demo`; consumes shared `conversationPanel` render/controller/style seams; canonical routes `BWP-R-001` through `BWP-R-020` under `/design-system/canonical-renderings/build-work-panel/*`; Playwright canonical coverage in `tests/visual/designSystem/canonicals/shell/buildWorkPanel.spec.ts`; root-admin UI-only adoption coverage in `tests/visual/app/rootAdminShell/rootAdminShellParity.spec.ts` | Connect root-admin to real Layer 1/harness APIs, persisted history, and permission-backed packet download before calling the product workflow complete | Root-admin shell Build panel |
