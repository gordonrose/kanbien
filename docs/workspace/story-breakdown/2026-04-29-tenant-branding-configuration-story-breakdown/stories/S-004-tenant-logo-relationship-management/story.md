...
| --- | --- | --- | --- | --- | --- |
| AC-S004-01 | root admin; asset system; tenant user as logo reader; authorized root; unauthorized actor; tenant user current context; pending asset; ready asset; rejected asset; deleted asset; tenant mismatch; missing alt metadata | root-admin.tenant-branding.logo.update; asset.create | runtime-api | TC obligation: cover asset contract; validation; quota; authz for Logo upload or replacement creates a short-lived, single-use, actor-bound, scope-bound, storage-key-bound intent for exactly one selected tenant using only the approved MIME and size limits. | yes |
| AC-S004-02 | root admin; asset system; tenant user as logo reader; authorized root; unauthorized actor; tenant user current context; pending asset; ready asset; rejected asset; deleted asset; tenant mismatch; missing alt metadata | root-admin.tenant-branding.logo.update; asset.link | persistence-level | TC obligation: cover asset readiness; state transition; accessibility metadata for The logo relationship can become consumer-ready only when the asset is ready, tenant scope matches, lifecycle state is allowed, and contextual alt text or decorative posture is recorded. | yes |
| AC-S004-03 | root admin; asset system; tenant user as logo reader; authorized root; unauthorized actor; tenant user current context; pending asset; ready asset; rejected asset; deleted asset; tenant mismatch; missing alt metadata | tenant-branding.logo.read; asset.content.read | runtime-api | TC obligation: cover asset content read; security headers; SVG safety for Dashboard logo delivery uses same-origin authenticated streaming with `nosniff`, never raw bucket URLs, never public delivery, and never direct DOM injection of uploaded SVG markup. | yes |
| AC-S004-04 | root admin; asset system; tenant user as logo reader; authorized root; unauthorized actor; tenant user current context; pending asset; ready asset; rejected asset; deleted asset; tenant mismatch; missing alt metadata | root-admin.tenant-branding.logo.update; asset.lifecycle.cleanup | persistence-level | TC obligation: cover lifecycle transition; cleanup evidence; audit for Replacement uses a new asset or version with a new storage key and leaves prior bytes governed by approved retention, cleanup, quota, and audit behavior. | yes |
