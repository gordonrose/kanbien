...
| --- | --- | --- | --- | --- | --- |
| AC-S003-01 | root admin; unauthorized actor; selected tenant present; missing selected tenant; unauthorized actor; no branding; active branding; soft-deleted branding | root-admin.tenant-branding.read | runtime-api | TC obligation: cover API contract; persistence integration; authz allow and deny for Root-admin read returns exactly one selected tenant branding record or the approved absence state while excluding soft-deleted records from normal reads. | yes |
| AC-S003-02 | root admin; unauthorized actor; selected tenant present; missing selected tenant; unauthorized actor; no branding; active branding; soft-deleted branding | root-admin.tenant-branding.manage | runtime-api | TC obligation: cover validation; authz; boundary values; API contract for Root-admin save rejects client-supplied system-managed fields, empty display names, invalid primary-colour values, missing selected tenant, and unauthorized actors. | yes |
| AC-S003-03 | root admin; unauthorized actor; selected tenant present; missing selected tenant; unauthorized actor; no branding; active branding; soft-deleted branding | root-admin.tenant-branding.manage; tenant-branding.audit.record | persistence-level | TC obligation: cover persistence integration; audit; regression for canonical tenant isolation for Successful root-admin save persists durable display name and primary colour as tenant branding facts, refreshes `updatedAt`, records audit evidence, and does not overwrite the canonical tenant name. | yes |
