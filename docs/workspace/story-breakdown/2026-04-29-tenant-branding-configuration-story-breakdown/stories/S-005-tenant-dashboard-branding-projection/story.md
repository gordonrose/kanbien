...
| --- | --- | --- | --- | --- | --- |
| AC-S005-01 | tenant user; unauthorized tenant actor; current tenant selected; no tenant context; wrong tenant context; complete branding; partial branding; no branding; not-ready logo; denied logo | tenant-branding.dashboard.read | runtime-api | TC obligation: cover tenant authz; cross-tenant deny; integration for Tenant dashboard branding read evaluates exactly one current tenant context and denies reads when the current tenant does not match the branding owner and asset tenant. | yes |
| AC-S005-02 | tenant user; unauthorized tenant actor; current tenant selected; no tenant context; wrong tenant context; complete branding; partial branding; no branding; not-ready logo; denied logo | tenant-branding.dashboard.read; tenant-branding.fallback.read | contract-level | TC obligation: cover API contract; state matrix; compatibility for The projection returns display name, primary colour, logo URL or null, logo accessibility posture, fallback indicators, and reload/login timing metadata using approved fallback behavior. | yes |
| AC-S005-03 | tenant user; unauthorized tenant actor; current tenant selected; no tenant context; wrong tenant context; complete branding; partial branding; no branding; not-ready logo; denied logo | tenant-branding.dashboard.read | runtime-api | TC obligation: cover session or projection refresh; compatibility for Branding changes are visible after next login or dashboard reload and no v1 behavior promises live updates to already-open dashboards. | yes |
