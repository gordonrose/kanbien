...
| --- | --- | --- | --- | --- | --- |
| AC-S002-01 | root operator; unauthorized actor; active root session; expired session; tenant active; tenant disabled; config active; config soft-deleted | root.tenant-auth-configuration.manage | runtime-api | TC obligation: cover authz; validation; audit for Root configuration writes require root authorization, exactly one target tenant, normalized configuration values, no client-supplied system-managed fields, and audit evidence. | yes |
| AC-S002-02 | root operator; unauthorized actor; active root session; expired session; tenant active; tenant disabled; config active; config soft-deleted | tenant-auth-configuration.persist | persistence-level | TC obligation: cover persistence integration; lifecycle review for Tenant auth configuration persists enabled method set, provider reference metadata, policy version, timestamps, lifecycle state, and force-login posture without depending only on mutable provider state. | yes |
