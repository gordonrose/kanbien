# T-S006-01: Create location table, lifecycle columns, optional latitude/longitude fields, address-summary field, descriptive office flags, and supporting indexes without head-office uniqueness.
...
| Execution Scope | Create location table, lifecycle columns, optional latitude/longitude fields, address-summary field, descriptive office flags, and supporting indexes without head-office uniqueness. |
| Allowed Write Set | `src/features/organizationLocations/persistence/**`; `tests/integration/organizationLocations/**`; shared Postgres harness only if required by migration proof. |
| Non-Goals | No API routes, no UI, no search implementation, no export job assembly, no hidden uniqueness for head-office flags. |
