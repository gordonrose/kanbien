...
| DEC-CHAT-012 | Classify `harnessChat` as discovery-harness for the current MVP, while keeping the physical implementation in `src/features/harnessChat`. | The current MVP supports Product Discovery interviews, story/packet generation, packet traceability, and root-admin harness chat workflows rather than general SaaS runtime behavior or code implementation. | No physical move is approved by this decision. Future promotion toward shared-governance-kernel requires a separate compatibility plan for routes, migrations, authz seeds, generated docs, and any retained history. | Harness/planning owner |
