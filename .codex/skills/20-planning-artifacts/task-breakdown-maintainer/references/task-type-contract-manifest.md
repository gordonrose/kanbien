...
| `DEV:frontend` | `frontend-task-guardrail.md` | Layer 2/3 frontend classification, signed-off `GOV:design-system` seam or approved exception, API/projection contract, security posture, performance posture, and allowed write set. | Implement one app/frontend behavior by consuming approved render, behavior, accessibility, and style seams without local drift. | App route/module changes, adoption contract, accessibility/security/runtime/mock-honesty evidence, visual or browser proof, and artifact obligations. | Make the missing-seam stop rule easier to apply before work starts, and strengthen coverage-strength interpretation for browser/visual proof depth. |
| `DEV:vertical-slice` | `vertical-slice-task-guardrail.md` | Approved evidence that one journey behavior requires backend and frontend proof together and cannot be split safely. | Deliver one inseparable journey behavior across backend and frontend seams. | Coupled API/data/browser proof, split rejection rationale, security/runtime/mock-honesty evidence, and carried artifact obligations. | Add clearer examples of valid versus invalid vertical-slice coupling so this type cannot become a broad implementation bucket. |
...
| `DECISION:refactor-first` | `refactor-first-task-guardrail.md` | Existing coupling, extraction, movement, or simplification pressure; affected consumers; downstream task blocked by the refactor. | Decide or perform one behavior-preserving prerequisite refactor before dependent work. | Compatibility-preserving refactor or decision output, existing-consumer proof, downstream unblocker, and no product behavior change. | Clarify when a refactor-first task becomes `DEV:platform-seam` or `GOV:architecture-update` because it changes shared seam authority. |
...
| `GOV:design-system` | `design-system-task-guardrail.md` | Governed family need, behavior lock or signoff source, canonical route, sub-standard, security/runtime/performance posture, and downstream adoption expectation. | Produce, refine, or prove one governed design-system seam for frontend consumption. | Consumable render, behavior, accessibility, and style/CSS seam; canonical proof; screenshot/visual/runtime evidence; and adoption contract. | Re-review under GOV nomenclature and add stronger examples for splitting fixture/data, visual, interaction, accessibility, and evidence-sweep work. |
