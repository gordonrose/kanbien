...
| `DECISION:architecture-foundation` | `architecture-foundation-task-guardrail.md` | Approved architecture concern area, approved trigger, exact architecture question, architecture sources to review, decision owner, output artifact target, blocked downstream tasks, compatibility posture, and final authority route. | Resolve or route one architecture blocker before implementation or governance work proceeds, without allowing Delivery to invent system authority. | Decision path, concern area, owner, reviewed source set, output artifact target, compatibility posture, downstream block/unblock record, and route to existing authority, Layer 2, ADR, `GOV:architecture-update`, or `GOV:standards-update`. | Add worked examples across ownership, integration, security/privacy, authz, persistence/data, compliance, frontend, design-system, scalability, resilience, operability, runtime, dependency, rollout, and testing-strategy concerns. |
