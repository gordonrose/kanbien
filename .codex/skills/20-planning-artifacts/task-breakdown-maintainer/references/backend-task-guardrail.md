...
| A protected mutation must emit an audit event after successful domain behavior. | `audit-event` | Source inventory names approved audit requirement, domain/transport files, audit helper or event seam, expected event payload, denied-path posture, and focused audit/security tests. | New audit taxonomy, retention, or observability policy routes to standards/architecture before implementation. |
| A write path must update two existing records atomically under an approved repository contract. | `transaction-consistency` | Inventory domain capability, repository methods, transaction boundary, failure/rollback expectations, idempotency posture, and persistence-backed tests. | New schema, indexes, or cleanup semantics split to migration-persistence or architecture tasks. |
| A feature-owned background job handler consumes an already-approved job payload. | `background-job-handler` | Name job payload contract, handler file, repository/domain consumers, retry/failure posture from approved source, and focused job/runtime tests. | Scheduler authority, retry policy, and shared job runtime changes belong to `DEV:platform-seam` or architecture. |
| Backend behavior emits metrics/logs without changing domain output. | `observability-event` | Inventory approved observability requirement, event/log/metric source, expected output, privacy redaction posture, and focused proof command. | New operational policy, alert thresholds, or compliance evidence route to architecture/standards/evidence tasks. |
