...
| `DEV:platform-seam` | strong | strong | medium | strong | strong | yes | medium | P2 | Add generator/materialization examples for route mounting, tooling harnesses, auth middleware, schedulers, and shared runtime helpers. |
...
| `DOC:api-contract` | medium | strong | medium | medium | strong | yes | medium | P2 | Add route-family examples and a scriptable maintained-artifact inventory for OpenAPI, Postman, generated docs, and no-wire-change contract refreshes. |
| `DOC:docs-artifact` | weak | medium | weak | medium | strong | yes | high | P0 | Add docs artifact classes, scriptable source inventory, exact docs targets, diff/check command, and human-review boundary. |
| `DOC:permission-mapping` | medium | strong | medium | medium | strong | yes | medium | P2 | Wait for the Layer 2 authz model, then harden configuration-based and relationship-based authorization rows with enforcement/test evidence mapping. |
...
| `DOC:standards-compliance` | weak | medium | medium | medium | strong | yes | high | P1 | Add deterministic external-control-map coverage summaries and examples for WCAG, GDPR, ISO, OWASP, and repo-only gate reviews. |
