...
| Existing behavior sits in a feature transport file but is already used as a domain-safe helper by multiple feature capabilities. | `wrong-owner-or-layer` / `move` | Move the existing helper to the approved feature-local owner, inventory all consumers, preserve exports or compatibility adapters, and prove all current consumers unchanged. | Do not broaden the helper into `src/lib` or platform infrastructure without architecture/seam authority. |
| A downstream task needs to reuse existing behavior, but the current function mixes reusable logic with request-specific parsing. | `extraction-before-reuse` / `extract` | Extract only the existing reusable behavior, leave request parsing in place, prove old route behavior unchanged, and name the downstream reuse task. | Do not add the new consuming behavior in the extraction task. |
| Existing implementation has a measurable hot path cost but public behavior, data shape, and contracts must remain identical. | `over-broad-write-set` / `performance-preserving` | Inventory the hot path, current consumers, benchmark or focused performance signal, compatibility tests, and rollback note; expected output is internal cost reduction only. | New caching policy, stale-data semantics, or architecture-level performance decisions route to architecture or platform-seam work. |
