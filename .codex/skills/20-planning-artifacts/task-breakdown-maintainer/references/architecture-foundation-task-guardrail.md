...
| A feature wants to store searchable multi-value attributes in JSON because it is easy, but query scale and operator support are undecided. | `persistence-data-model` / `persistence-model` | Record storage/index/query decision gap, reviewed ADRs/AGENTS storage rules, missing scale/operator analysis, owner, output route, and blocked migration/backend tasks. | Do not choose JSON, arrays, or junction tables in Layer 4; route approved storage authority to Layer 2 or `GOV:architecture-update`. |
| A new external provider, package, or runtime helper may become a durable dependency. | `dependency-selection` / `architecture-source-gap` | Capture build-vs-buy question, alternatives, security/operability/reversibility gaps, source inventory, decision owner, and downstream tasks blocked. | Do not install or wire the dependency until the decision source is approved. |
| Proof ownership is unclear between integration tests, e2e browser proof, and QA evidence for a sensitive journey. | `testing-strategy-architecture` / `architecture-source-gap` | Record proof-layer question, source PRD/test strategy docs reviewed, missing testability/signoff fields, and final authority route. | Do not relabel proof tasks or weaken evidence requirements inside the decision task. |
