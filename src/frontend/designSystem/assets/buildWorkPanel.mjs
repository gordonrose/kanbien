...
export const buildWorkPanelCanonicalRefs = [
  {
    ref: "BWP-R-001",
...
export function getBuildWorkPanelCanonicalRef(refId = "BWP-R-002") {
  return buildWorkPanelCanonicalRefs.find((entry) => entry.ref === refId) ?? buildWorkPanelCanonicalRefs[1];
}
    ...
    return `
      <section class="build-work-panel-demo-packet" data-build-work-panel-packet aria-label="Product Discovery packet status">
        <div class="build-work-panel-demo-packet-row">
          <div>
            <strong>Product Discovery packet</strong>
            <p class="build-work-panel-demo-review-note build-work-panel-demo-review-note-denied"><span class="build-work-panel-demo-status-dot" aria-hidden="true"></span><span>Restricted</span></p>
            <p class="build-work-panel-demo-review-note">You do not have access to download this packet from the current context.</p>
          </div>
        </div>
      </section>
    `;
    ...
    return `
      <section class="build-work-panel-demo-packet" data-build-work-panel-packet aria-label="Product Discovery packet status">
        <div class="build-work-panel-demo-packet-row">
          <div>
            <strong>Product Discovery packet</strong>
            <p class="build-work-panel-demo-review-note build-work-panel-demo-review-note-failed"><span class="build-work-panel-demo-status-dot" aria-hidden="true"></span><span>Download preparation failed</span></p>
            <p class="build-work-panel-demo-review-note">The approved packet is still preserved in history.</p>
          </div>
          <button class="build-work-panel-demo-repeat" type="button" data-build-work-panel-download aria-label="Retry packet download">${svg(iconPaths.retry)}</button>
        </div>
      </section>
    `;
  }
  ...
  return `
    <section class="build-work-panel-demo-packet" data-build-work-panel-packet aria-label="Product Discovery packet status">
      <div class="build-work-panel-demo-packet-row">
        <div>
          <strong>Product Discovery packet</strong>
          <p class="build-work-panel-demo-review-note"><span class="build-work-panel-demo-status-dot" aria-hidden="true"></span><span data-build-work-panel-download-status>Ready to download</span></p>
          <p class="build-work-panel-demo-review-note">Approved packet data only. Version 1 of 1.</p>
        </div>
        <button class="build-work-panel-demo-download" type="button" data-build-work-panel-download>Download PDF</button>
      </div>
    </section>
  `;
}
...
export function renderBuildWorkPanel(
  ...
  root.innerHTML = `
    <aside class="build-work-panel-demo-panel${panelOpen ? " is-open" : ""}" data-build-work-panel-panel data-history-open="${historyOpen ? "true" : "false"}" aria-label="Build work panel">
      <header class="build-work-panel-demo-panel-header">
        ...
        <div class="build-work-panel-demo-header-actions">
          ...
          <button class="build-work-panel-demo-close" type="button" data-build-work-panel-close aria-label="Close Build work panel">${svg(iconPaths.close)}</button>
        </div>
      </header>
    ...
    <nav class="build-work-panel-demo-action-nav" aria-label="Page actions">
      <button class="build-work-panel-demo-action" type="button" aria-disabled="true" data-tooltip="Reporting">${svg(iconPaths.reporting)}<span>Reporting</span></button>
      <button class="build-work-panel-demo-action" type="button" aria-disabled="true" data-tooltip="Support">${svg(iconPaths.support)}<span>Support</span></button>
      <button class="build-work-panel-demo-action" type="button" aria-pressed="true" data-build-work-panel-build-action data-tooltip="Build">${svg(iconPaths.build)}<span>Build</span></button>
    </nav>
...
export function createBuildWorkPanelController(root, options = {}) {
  renderBuildWorkPanel(root, options);
