...
const entityManagementLazyRenderers = new Map();

function registerEntityManagementLazyRenderer(key, renderer) {
  entityManagementLazyRenderers.set(key, renderer);
  return key;
}

function renderEntityManagementLazyPlaceholder(key) {
  return `<template data-entity-management-lazy-panel="${escapeHtml(key)}"></template>`;
}

function materializeEntityManagementLazyPanel(panel) {
  if (!(panel instanceof HTMLElement) || panel.dataset.entityManagementLazyRendered === "true") {
    return false;
  }
  const lazyKey = panel.dataset.entityManagementLazyPanelKey;
  const renderer = lazyKey ? entityManagementLazyRenderers.get(lazyKey) : null;
  if (typeof renderer !== "function") {
    return false;
  }
  panel.innerHTML = renderer() ?? "";
  panel.dataset.entityManagementLazyRendered = "true";
  initializeFormDrawerSelects({ scope: panel });
  const drawer = panel.closest("[data-chat-workspace-list-drawer]");
...
function renderRecordManagementRegionShell({ label, regions }) {
  const visibleRegions = regions.filter((region) => region?.content || region?.renderContent);
    ...
    <div class="record-management-region-shell" data-record-management-region-shell>
      ...
      <div class="record-management-region-panels">
        ${visibleRegions
          .map((region) => {
            const isActive = region.key === activeKey;
            const lazyKey = region.lazyKey ?? `region:${region.key}`;
            const content = isActive
              ? (typeof region.renderContent === "function" ? region.renderContent() : region.content)
              : renderEntityManagementLazyPlaceholder(lazyKey);
            return `
              <section
                ...
                data-entity-management-lazy-panel-key="${escapeHtml(lazyKey)}"
                data-entity-management-lazy-rendered="${isActive ? "true" : "false"}"
                ${isActive ? "" : "hidden"}
              >
                ${content ?? ""}
              </section>
...
function initializeEntityManagementPageBehavior(drawer) {
  function initializeNestedList(nestedList) {
    if (!(nestedList instanceof HTMLElement) || nestedList.dataset.recordManagementNestedListInitialized === "true") {
      return;
    }
    nestedList.dataset.recordManagementNestedListInitialized = "true";
  ...
  function initializeLazyRegionContent(panel) {
    if (!(panel instanceof HTMLElement)) {
      return;
    }
    if (materializeEntityManagementLazyPanel(panel)) {
      panel.querySelectorAll("[data-record-management-nested-list]").forEach((nestedList) => {
        initializeNestedList(nestedList);
      });
    }
  }
  ...
  function activateRegion(shell, key) {
    ...
    shell.querySelectorAll("[data-record-management-region-panel]").forEach((panel) => {
      ...
      const isActive = panel.dataset.recordManagementRegionPanel === key;
      if (isActive) {
        initializeLazyRegionContent(panel);
      }
...
function activateNestedListItem(nestedList, key) {
  ...
  nestedList.querySelectorAll("[data-record-management-nested-panel]").forEach((panel) => {
    ...
    const isActive = panel.dataset.recordManagementNestedPanel === key;
    if (isActive) {
      materializeEntityManagementLazyPanel(panel);
    }
    panel.hidden = !isActive;
  });
}
...
function renderNestedListPicker({ addAction = null, className = "", label, description, items, scopeKey = label }) {
  const activeKey = items[0]?.key ?? "";
  const sectionClass = `record-management-nested-list${className ? ` ${className}` : ""}`;
  const normalizedScopeKey = String(scopeKey)
    ...
    .replaceAll(/^-|-$/g, "");
  return `
    <section class="${escapeHtml(sectionClass)}" aria-label="${escapeHtml(label)}" data-record-management-nested-list>
      ...
      <div class="record-management-nested-list-layout">
        ...
        <div class="record-management-nested-list-drawer">
          ${items.map((item) => {
            ...
            const lazyKey = hasCustomContent
              ? (item.lazyKey ?? registerEntityManagementLazyRenderer(
                `nested:${normalizedScopeKey}:${item.key}`,
                () => (typeof item.content === "function" ? item.content() : item.content),
              ))
              : "";
            const content = hasCustomContent && isActive
              ? (typeof item.content === "function" ? item.content() : item.content)
              : (hasCustomContent ? renderEntityManagementLazyPlaceholder(lazyKey) : "");
            return `
              <section
                data-record-management-nested-panel="${escapeHtml(item.key)}"
                ${hasCustomContent ? `
                  data-entity-management-lazy-panel-key="${escapeHtml(lazyKey)}"
                  data-entity-management-lazy-rendered="${isActive ? "true" : "false"}"
                ` : ""}
...
function renderEntityManagementIdentityRegion() {
  return renderNestedListPicker({
    className: "entity-management-identity-region",
    label: "Identity",
    description: "Definition identity fields, feature ownership, and source authority posture.",
    items: [
      {
        key: "primary-details",
        label: "Primary Details",
  ...
  const regions = [
    {
      ...
      count: 3,
      renderContent: renderEntityManagementIdentityRegion,
    },
    {
      key: "workflows",
      label: "Workflows",
      ...
      count: 3,
      renderContent: renderEntityManagementWorkflowsRegion,
    },
    {
      key: "views",
      label: "Views",
      ...
      count: 3,
      renderContent: renderEntityManagementViewsRegion,
    },
    {
      key: "relationships",
      label: "Relationships",
      ...
      count: 3,
      renderContent: renderEntityManagementRelationshipsRegion,
    },
    {
      key: "attributes",
      label: "Attributes",
      ...
      count: 6,
      renderContent: renderEntityManagementAttributesRegion,
    },
    {
      key: "catalogs",
      label: "Catalogs",
      ...
      count: 4,
      renderContent: renderEntityManagementCatalogsRegion,
    },
    {
      key: "placements",
      label: "Display",
      ...
      renderContent: renderEntityManagementPlacementsRegion,
    },
    {
      key: "permissions",
      label: "Permissions",
      ...
      count: 1,
      renderContent: renderEntityManagementPermissionsRegion,
    },
    {
      ...
      count: 4,
      renderContent: renderEntityManagementGenerationModelRegion,
    },
    {
      ...
      count: 4,
      renderContent: renderEntityManagementComplianceModelRegion,
    },
    {
      key: "migration-model",
      ...
      count: 4,
      renderContent: renderEntityManagementMigrationModelRegion,
    },
    {
      ...
      renderContent: renderEntityManagementActionModelsRecordRegion,
    },
    {
      ...
      renderContent: renderEntityManagementActionModelsEntityStructureRegion,
    },
  ];

  regions.forEach((region) => {
    if (typeof region.renderContent === "function") {
      region.lazyKey = registerEntityManagementLazyRenderer(`region:${region.key}`, region.renderContent);
    }
  });
