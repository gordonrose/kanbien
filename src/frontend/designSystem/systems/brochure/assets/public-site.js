(function () {
  function activateTab(tabList, nextTab) {
    const showcase = tabList.closest("[data-public-site-showcase]");
    const select = showcase ? showcase.querySelector("[data-showcase-select]") : null;
  ...
  for (const showcase of document.querySelectorAll("[data-public-site-showcase]")) {
    ...
    const tabs = Array.from(tabList.querySelectorAll("[data-showcase-tab]"));
    const select = showcase.querySelector("[data-showcase-select]");
    ...
    if (select) {
      select.addEventListener("change", () => {
        const selectedTab = document.getElementById(select.value);

        if (selectedTab) {
          activateTab(tabList, selectedTab);
        }
      });
    }
  }
